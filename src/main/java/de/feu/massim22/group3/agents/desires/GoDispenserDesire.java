...
public class GoDispenserDesire extends BeliefDesire {
    ...
    public ActionInfo getNextActionInfo() {
        ...
        Point visionDispenser = Point.castToPoint(belief.getNearestRelativeManhattanDispenser(block.type));
        ...
        if (visionDispenser != null) {
            ...
            if (agent.requestMade && agent.lastUsedDispenser != null && agent.lastUsedDispenser != nearestDispenser.position()) {
                ...
                for (Thing thing : agent.getBelief().getThings()) {
                    ...
                    if (thing.type.equals(Thing.TYPE_DISPENSER)
                            && thing.x == agent.lastUsedDispenser.x - agent.getBelief().getPosition().x
                            && thing.y == agent.lastUsedDispenser.y - agent.getBelief().getPosition().y) {
            ...
            distance = Math.abs(dispenserItself.x - agent.getBelief().getPosition().x)
                    + Math.abs(dispenserItself.y - agent.getBelief().getPosition().y);
            visionDispenser = dispenserItself.sub(Point.castToPoint(agent.getBelief().getPosition()));
        ...
        if (distance == 1) {
            ...
            if (attachPossible) {
                ...
                for (Meeting meeting : AgentMeetings.find(agent)) {
                    ...
                    AgentLogger.info(Thread.currentThread().getName() + "Test.getNextAction() 8.1: " + meeting.agent2().getName() + " , " + Point.castToPoint(meeting.agent2().getBelief().getPosition()).add(p1.add(p3.sub(p2))));      
                    
                    if (d1 == Point.castToPoint(meeting.agent2().getBelief().getPosition()).add(p1.add(p3.sub(p2))) && meeting.agent2().getBelief().getAttachedPoints().contains(r1)
                            || d2 == Point.castToPoint(meeting.agent2().getBelief().getPosition()).add(p1.add(p3.sub(p2))) && meeting.agent2().getBelief().getAttachedPoints().contains(r2)
                            || d3 == Point.castToPoint(meeting.agent2().getBelief().getPosition()).add(p1.add(p3.sub(p2))) && meeting.agent2().getBelief().getAttachedPoints().contains(r3)) {
