...
public class GoGoalZoneDesire extends BeliefDesire {
		...
		if (belief.getRole().actions().contains(Actions.DETACH)
	        ...
	        if (!result) {
		        ...
		        for (Meeting meeting : AgentMeetings.find(agent)) {
                    AgentLogger.info(Thread.currentThread().getName() + " Test.GoalZone 4 - agent: " + meeting.agent2().getName() + " , rgz: " + meeting.agent2().getBelief().getReachableGoalZones());
			        
                    if (!meeting.agent2().getBelief().getReachableGoalZones().isEmpty()) {
                        Point p1 = new Point(meeting.posAgent1());
                        Point p2 = new Point(meeting.posAgent2());
                        Point p3 = new Point(meeting.relAgent2());
                        Point nearestGoalZone = Point.castToPoint(meeting.agent2().getBelief().getNearestGoalZone().position())
                                .add(p1.add(p3.sub(p2)));
                        AgentLogger.info(Thread.currentThread().getName() + " Test.GoalZone 5: " + meeting.agent2().getBelief().getNearestGoalZone() + " , dist: " + Point.distance(Point.castToPoint(agent.getBelief().getPosition()), nearestGoalZone) + " , min: " + distance);
	                    
	                    if (Point.distance(Point.castToPoint(agent.getBelief().getPosition()), nearestGoalZone) < distance) {
                            ...
                            nearestGoalZoneGoalZoneAgentNew = Point.castToPoint(meeting.agent2().getBelief().getNearestGoalZone().position());
                            distance = Point.distance(Point.castToPoint(agent.getBelief().getPosition()), nearestGoalZone);
    	                    result = true;
			            }
			        }
			    }
    ...
    public ActionInfo getNextActionInfo() {
        if (goalZoneAgent == null) {
            ...
            String direction = DirectionUtil.getDirection(agent.getBelief().getPosition(), nearestGoalZoneAgentNew);
            int distance = Point.distance(Point.castToPoint(agent.getBelief().getPosition()), nearestGoalZoneAgentNew);
            AgentLogger.info(Thread.currentThread().getName() + "GoGoalZoneDesire - GoalZone from strange agent: "
