	...
	public void setNextAction() {		
		...
		for(Desire desire : allGoalsList) {
			for(SubDesire subDesire : desire.getSubDesires()) {
				// buffer precondition (so it is not calculated multiple times for each if clause)
				boolean isExecuteable = subDesire.isExecutable();
				// buffer postcondition (so it is not calculated multiple times for each if clause)
				boolean isDone = subDesire.isDone();
				
				// If the preconditions of one SubDesire are not met then all subsequent SubDesires of the Desire
				// cannot be executed because they depend on the postconditions of the previous SubDesires of one Desire
				// If the SubDesire is not executable but done it is OK. The next SubDesire can be executed.
				if (!isExecuteable && !isDone) {
					AgentLogger.fine(agent.getName(), 
							String.format(
				...
				// Execute SubDesire if preconditions (isExecutable()) are fulfilled but the postconditions (isDone()) not
				else if (isExecuteable && !isDone) {
					AgentLogger.info(agent.getName(),
							String.format("Exectung SubDesire %s of Desire %s",
									subDesire.toString(),
									desire.toString()));
